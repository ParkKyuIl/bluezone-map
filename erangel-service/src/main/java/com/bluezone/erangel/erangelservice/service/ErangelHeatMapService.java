package com.bluezone.erangel.erangelservice.service;
...
public class ErangelHeatMapService {
    ...
    private double[] linspace() {
        double[] result = new double[ErangelHeatMapService.GRID_SIZE];
        double step = ((double) ErangelHeatMapService.MAP_SIZE - (double) 0) / (ErangelHeatMapService.GRID_SIZE - 1);
        for (int i = 0; i < ErangelHeatMapService.GRID_SIZE; i++) result[i] = (double) 0 + step * i;
        return result;
    }
